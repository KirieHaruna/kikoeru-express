...
const createSchema = () => knex.schema
  ...
  .createTable('t_history', (table) => {
    ...
    table.string('work_id', 255).notNullable();
    table.string('hash', 255);
    table.string('track_name', 255);
    table.decimal('play_time');
  ...
  .raw(`
    ...
    FROM (
      SELECT baseQuery.*
        , json_object('vas', json_group_array(DISTINCT json_object('id', t_va.id, 'name', t_va.name))) AS vaObj
        , CASE 
          WHEN COUNT(t_history.user_name) > 0 THEN json_object('history', json_group_array(json_object('play_time', t_history.play_time, 'user_name', t_history.user_name, 'track_name', t_history.track_name)))
          ELSE NULL
        END AS hisObj
      FROM (
        SELECT t_work.id, t_work.title, t_work.circle_id, t_circle.name
          , json_object('id', t_work.circle_id, 'name', t_circle.name) AS circleObj
          , t_work.nsfw, t_work.release, t_work.dl_count, t_work.price, t_work.created_at
          , t_work.lrc, t_work.review_count, t_work.rate_count, t_work.rate_average_2dp, t_work.rate_count_detail
          , t_work.rank
        FROM t_work
        ...
        LEFT JOIN t_history t_history ON t_history.work_id = baseQuery.id
